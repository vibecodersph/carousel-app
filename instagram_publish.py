...
def channel_publishing(channel_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {}
    return channel.publishing if isinstance(channel.publishing, dict) else {}
...
def graph_request(
    ...
    *,
    ...
    try:
        ...
        raise SystemExit(f"{api_name} Graph API error {exc.code}: {message}") from exc
    except (OSError, json.JSONDecodeError) as exc:
        raise SystemExit(f"{api_name} Graph API request failed: {exc}") from exc
